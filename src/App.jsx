...
import { 
  ...
  History, MousePointer2, RefreshCw, Server, Database, ChevronDown, Menu, Clock
} from 'lucide-react';
import {
...
const App = () => {
  ...
  /**
   * 서버(DB)에서 최신 데이터를 가져오는 함수 (Cold Start 대응 포함)
   */
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      setStatusMessage({ type: 'error', text: '서버에 연결할 수 없습니다. 서버 상태를 확인하세요.' });
    } finally {
  ...
  const handleFileUpload = async (event) => {
    ...
    setIsUploading(true);
    setStatusMessage({ type: 'info', text: '데이터 전송을 시작합니다...' });

    try {
      for (let i = 0; i < files.length; i++) {
        const formData = new FormData();
        formData.append('excelFile', files[i]); // 서버의 multer가 'excelFile' 키로 기다리고 있습니다.

        setStatusMessage({ type: 'info', text: `[${i+1}/${files.length}] ${files[i].name} 전송 중...` });
        ...
        if (!response.ok) {
          ...
          throw new Error(errorData.error || `${files[i].name} 업로드에 실패했습니다.`);
        }
      }

      setStatusMessage({ type: 'success', text: '모든 데이터가 성공적으로 서버 DB에 저장되었습니다!' });
      await fetchDashboardData(); // 업로드 성공 후 즉시 데이터 갱신

    } catch (error) {
      console.error("Upload error:", error);
      setStatusMessage({ type: 'error', text: error.message || '데이터 전송 중 오류가 발생했습니다.' });
    } finally {
      setIsUploading(false);
      event.target.value = null; // 인풋 초기화
    }
  };
  ...
  const clearData = async () => {
    if (window.confirm("데이터베이스의 모든 기록을 삭제하시겠습니까?")) {
      try {
        const response = await fetch(`${SERVER_URL}/api/clear`, { method: 'DELETE' });
        if (!response.ok) throw new Error('삭제 실패');
        setStatusMessage({ type: 'success', text: '데이터베이스가 성공적으로 비워졌습니다.' });
        fetchDashboardData();
      } catch(e) { 
        setStatusMessage({ type: 'error', text: '초기화 작업 중 오류가 발생했습니다.' });
      }
    }
  };
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-slate-800 font-sans">
      ...
      <aside className={`fixed left-0 top-0 h-full bg-white border-r border-slate-200 z-30 flex flex-col transition-all duration-300 ${isSidebarCollapsed ? 'w-20' : 'w-64'}`}>
        ...
        <nav className="flex-1 px-4 py-6 space-y-2">
          ...
          <button onClick={() => setActiveTab('products')} className={`w-full flex items-center gap-4 p-3.5 rounded-2xl transition-all ${activeTab === 'products' ? 'bg-emerald-50 text-emerald-700 shadow-sm font-bold' : 'text-slate-400 hover:bg-slate-50'}`}>
            <ShoppingCart size={20} /> {!isSidebarCollapsed && <span>상품분석 리스트</span>}
          </button>
        </nav>
        ...
        <div className="p-4 space-y-3 border-t border-slate-50">
          <div className="relative bg-slate-50 p-5 rounded-[24px] border border-slate-100 flex flex-col items-center gap-3 hover:bg-emerald-50 transition-all cursor-pointer group shadow-sm overflow-hidden">
            <input type="file" multiple accept=".xlsx" onChange={handleFileUpload} className="absolute inset-0 opacity-0 cursor-pointer z-10" title="파일 선택" disabled={isUploading || hasConnectionError} />
            ...
            {!isSidebarCollapsed && (
              <span className="text-xs font-black text-slate-600 text-center">
                {isUploading ? '데이터 처리 중...' : '엑셀 데이터 업로드'}
              </span>
            )}
          </div>
            ...
            <div className={`px-3 py-3 rounded-xl flex items-center gap-3 shadow-inner text-left ${hasConnectionError ? 'bg-rose-50 border border-rose-100' : 'bg-emerald-50 border border-emerald-100'}`}>
               ...
               <div className="min-w-0">
                  ...
                  <p className={`text-[10px] font-bold truncate ${hasConnectionError ? 'text-rose-800' : 'text-emerald-800'}`}>
                    {hasConnectionError ? '서버 연결 실패' : 'DB 서버 동기화'}
                  </p>
               </div>
            </div>
          )}
          ...
          {processedData.length > 0 && !isSidebarCollapsed && (
            <button onClick={clearData} className="w-full flex items-center justify-center gap-2 p-3 text-rose-400 hover:text-rose-600 hover:bg-rose-50 rounded-xl text-xs font-bold transition-all underline underline-offset-4">DB 초기화</button>
          )}
        </div>
      </aside>
        ...
        <header className="h-20 bg-white/80 backdrop-blur-xl sticky top-0 z-20 flex items-center justify-between px-10 border-b border-slate-100">
          <div className="flex items-center gap-4 text-left">
             <h2 className="text-xl font-black text-slate-900 tracking-tight leading-none">
               {activeTab === 'dashboard' ? '서버 분석 리포트' : '전체 상품 분석'}
             </h2>
             <div className="flex items-center gap-2">
               ...
               <span className="text-[10px] bg-slate-100 text-slate-600 px-2 py-1 rounded font-black uppercase tracking-widest">Server Integrated</span>
             </div>
          </div>
          ...
          {isFetching && processedData.length === 0 ? (
            <div className="h-[60vh] flex flex-col items-center justify-center">
              ...
              <p className="font-black text-slate-400 tracking-widest uppercase text-xs mt-4">
                {isWakingUp ? '서버가 잠에서 깨어나는 중입니다 (최대 1분)...' : '데이터를 불러오고 있습니다...'}
              </p>
            </div>
          ) : hasConnectionError ? (
            <div className="h-[70vh] flex flex-col items-center justify-center text-slate-400 border-2 border-dashed border-rose-200 rounded-[56px] bg-white shadow-2xl p-10">
              ...
              <h3 className="text-2xl font-black text-slate-900 mb-2 italic">서버 연결이 원활하지 않습니다.</h3>
              <p className="text-slate-500 font-medium text-center mb-6">서버 주소가 정확한지, Render 서버가 켜져 있는지 확인해 주세요.</p>
            ...
            <div className="h-[70vh] flex flex-col items-center justify-center text-slate-300 border-2 border-dashed border-slate-200 rounded-[56px] bg-white shadow-2xl">
              ...
              <h3 className="text-2xl font-black text-slate-900 mb-2 italic">분석할 데이터가 없습니다.</h3>
              <p className="text-slate-400 font-medium">왼쪽 하단의 업로드 버튼을 통해 엑셀 파일을 서버로 전송해 주세요.</p>
            </div>
          ) : (
            <>
              {/* 통계 요약 카드 */}
                ...
                {[
                  ...
                  <div key={i} className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-xl shadow-slate-200/40 hover:translate-y-[-4px] transition-all group text-left">
                    <div className="flex justify-between items-start mb-6">
                      ...
                      <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">PRO METRIC</span>
                    </div>
                ...
                <div className="space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-700">
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    <div className="bg-white p-10 rounded-[48px] border border-slate-100 shadow-xl text-left">
                      <h3 className="font-black text-lg mb-8 flex items-center gap-3"><div className="w-1.5 h-6 bg-emerald-600 rounded-full"></div> 월간 매출 성장</h3>
                      <div className="h-72">
                    ...
                    <div className="bg-white p-10 rounded-[48px] border border-emerald-100 shadow-xl shadow-emerald-50/50 text-left">
                      <h3 className="font-black text-lg mb-8 flex items-center gap-3 text-emerald-700"><div className="w-1.5 h-6 bg-emerald-500 rounded-full"></div> 월간 유입수 변화</h3>
                      <div className="h-72">
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart data={monthlyTrend}>
                            <CartesianGrid strokeDasharray="0" vertical={false} stroke="#F1F5F9" />
                  ...
                  <div className="bg-white p-10 rounded-[48px] border border-slate-100 shadow-xl text-left">
                    <h3 className="font-black text-lg flex items-center gap-3 text-emerald-500 mb-10"><div className="w-1.5 h-6 bg-emerald-400 rounded-full"></div> 일별 유입 흐름 (Time-series)</h3>
                    <div className="h-96">
                      <ResponsiveContainer width="100%" height="100%">
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="border-b border-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-widest bg-slate-50/20 text-left">
                          <th className="px-10 py-6 text-left">상품명 / ID</th>
                          {[{ label: '조회수', key: '상품상세조회수' }, { label: '주문량', key: '결제상품수량' }, { label: '매출액', key: '결제금액' }, { label: '전환율', key: '상세조회대비결제율' }].map(col => (
                            <th key={col.key} className="px-6 py-6 cursor-pointer hover:text-emerald-600 group transition-colors text-left" onClick={() => handleSort(col.key)}>
                      ...
                      <tbody className="divide-y divide-slate-50 text-left">
                        {sortedData.slice(0, visibleCount).map((item, idx) => (
                          <tr key={idx} className="hover:bg-slate-50/50 cursor-pointer group transition-colors text-left" onClick={() => setSelectedProduct(item)}>
                            <td className="px-10 py-8 min-w-[350px] text-left">
                              <div className="font-black text-slate-900 group-hover:text-emerald-600 transition-colors flex items-start gap-2 whitespace-normal break-all max-w-[450px] leading-relaxed text-left">
                                {String(item.lastName || '이름 없음')} {Number(item.nameCount) > 1 && <span className="bg-amber-100 text-amber-700 text-[9px] px-2 py-0.5 rounded font-black mt-1 uppercase shrink-0 text-left">MODIFIED</span>}
                              </div>
                            ...
                            <td className="px-6 py-8 text-left"><span className="bg-emerald-50 text-emerald-600 px-3 py-1 rounded-lg text-xs font-black text-left">{(Number(item.상세조회대비결제율) * 100).toFixed(2)}%</span></td>
                            <td className="px-10 py-8 text-center text-left"><div className="w-12 h-12 rounded-2xl bg-white border border-slate-100 flex items-center justify-center text-slate-300 group-hover:bg-emerald-600 group-hover:text-white transition-all mx-auto"><ChevronRight size={18} /></div></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
          ...
          <div className="bg-white w-full max-w-7xl max-h-[94vh] rounded-[64px] shadow-2xl border-4 border-white flex flex-col overflow-hidden animate-in zoom-in-95 duration-500">
            <div className="px-12 py-12 flex items-center justify-between border-b border-slate-50 text-left">
              <div className="flex items-center gap-10">
                <div className="bg-emerald-600 p-6 rounded-[32px] text-white shadow-2xl"><Eye size={36} /></div>
                <div className="max-w-[800px] text-left">
                  <h3 className="text-3xl font-black text-slate-900 leading-tight whitespace-normal break-words tracking-tighter italic text-left">{String(selectedProduct.lastName || '')}</h3>
                  <p className="text-xs text-slate-400 font-black mt-2 tracking-[0.3em] uppercase opacity-60 italic text-left">PID: {String(selectedProduct.상품ID || '')}</p>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-12 py-12 space-y-16 text-left">
                    ...
                    <table className="w-full text-left text-sm">
                      <thead>
                        <tr className="bg-slate-50/60 text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">
                          <th className="px-10 py-6 text-left">상품명</th>
                          <th className="px-4 py-6 text-center text-left">사용 기간</th>
                          <th className="px-4 py-6 text-right text-left">매출액</th>
                          <th className="px-4 py-6 text-right italic text-emerald-600 text-left">일평균 매출</th>
                          <th className="px-10 py-6 text-right text-left">전환율</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-50">
                        {selectedProduct.performanceByName.map((p, i) => (
                          <tr key={i} className="hover:bg-slate-50/30 transition-colors font-black text-left">
                            <td className="px-10 py-6 font-bold text-slate-800 italic">{String(p.name || '')}</td>
                            <td className="px-4 py-6 text-center text-slate-400 text-[11px] text-left">
                              {String(p.periodStart || '').replace(/-/g,'.')} - {String(p.periodEnd || '').replace(/-/g,'.')}
                            </td>
                            <td className="px-4 py-6 text-right text-left">₩{(Number(p.totalRevenue) || 0).toLocaleString()}</td>
                            <td className="px-4 py-6 text-right text-emerald-600 text-left">₩{Math.round(Number(p.dailyAvgRevenue) || 0).toLocaleString()}</td>
                            <td className="px-10 py-6 text-right text-left">{(Number(p.cvr) || 0).toFixed(2)}%</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              {Array.isArray(selectedProduct.history) && selectedProduct.history.length > 0 && (
                <div className="h-80 bg-white p-10 rounded-[56px] border border-slate-100 shadow-2xl text-left">
                  <h4 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-6 px-4">상품 유입 변화 추이</h4>
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={selectedProduct.history}>
                      ...
                      <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#F1F5F9" /><XAxis dataKey="date" hide /><YAxis hide /><Tooltip contentStyle={{borderRadius: '24px', border: 'none'}} /><Area name="조회수" type="monotone" dataKey="조회수" stroke="#10B981" strokeWidth={5} fillOpacity={1} fill="url(#colorProdViews)" />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
              )}
            </div>
            <div className="p-12 bg-white border-t border-slate-50 flex justify-center text-left"><button onClick={() => setSelectedProduct(null)} className="px-32 py-6 bg-slate-900 text-white rounded-[32px] font-black text-lg hover:scale-105 transition-all shadow-xl shadow-slate-200">데이터 창 닫기</button></div>
          </div>
        </div>
      )}
      ...
      {statusMessage && (
        <div className={`fixed bottom-10 right-10 px-8 py-5 rounded-[28px] shadow-2xl text-white font-black flex items-center gap-4 animate-in slide-in-from-bottom-8 z-[200] ${statusMessage.type === 'error' ? 'bg-rose-500' : statusMessage.type === 'info' ? 'bg-indigo-600' : 'bg-slate-900'}`}>
          ...
          <span className="tracking-tight leading-none text-left">{String(statusMessage.text)}</span>
