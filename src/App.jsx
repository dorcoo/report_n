...
import { getFirestore, doc, setDoc, getDoc, collection, getDocs, writeBatch, query, deleteDoc } from 'firebase/firestore';
...
const App = () => {
      ...
      try {
        ...
        if (metaSnap.exists() && window.LZString) {
          ...
          for (let i = 0; i < chunkCount; i++) {
            const chunkRef = doc(db, 'artifacts', appId, 'users', user.uid, 'payloads', `chunk_${i}`);
            const chunkSnap = await getDoc(chunkRef);
            if (chunkSnap.exists()) {
              fullPayload += chunkSnap.data().data;
            }
          }
          ...
          if (decompressed) {
            ...
            setMonthlyTrend(parsed.monthlyTrend || []);
            setGlobalMaxDate(parsed.globalMaxDate || '');
            setStatusMessage({ type: 'success', text: '대용량 클라우드 데이터를 불러왔습니다.' });
          }
        }
  ...
  const performCloudSync = async (dataObj) => {
    ...
    try {
      ...
      // 약 800KB 단위로 쪼개기 (안전을 위해 1MB보다 작게 설정)
      ...
      // 1. 기존 조각들 삭제 (초기화)
      const payloadsCol = collection(db, 'artifacts', appId, 'users', user.uid, 'payloads');
      ...
      setStatusMessage({ type: 'error', text: '클라우드 저장 실패: 네트워크 환경을 확인해주세요.' });
    }
  ...
  const processFiles = async (targetFiles) => {
    ...
    try {
      ...
      setStatusMessage({ type: 'success', text: `누적 분석 및 대용량 클라우드 저장 완료!` });
    } catch (err) { setStatusMessage({ type: 'error', text: '데이터 처리 중 오류가 발생했습니다.' }); }
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-slate-800 font-sans selection:bg-indigo-100">
        ...
        <header className="h-20 bg-white/80 backdrop-blur-xl sticky top-0 z-20 flex items-center justify-between px-10 border-b border-slate-100">
          <div className="flex items-center gap-4">
             ...
             <div className="flex items-center gap-2">
               ...
               <span className="text-[10px] bg-slate-100 text-slate-600 px-2 py-1 rounded-md font-black uppercase tracking-widest">대용량 모드 활성</span>
               {isSyncing && <div className="flex items-center gap-1 text-[10px] text-indigo-500 font-bold animate-pulse"><RefreshCw size={10} className="animate-spin" /> 클라우드 분할 저장 중...</div>}
             </div>
          </div>
        ...
        <div className="p-10 max-w-[1400px] mx-auto space-y-10">
          {processedData.length === 0 ? (
            <div className="h-[70vh] flex flex-col items-center justify-center text-slate-300 border-2 border-dashed border-slate-200 rounded-[56px] bg-white shadow-2xl shadow-slate-100">
              ...
              <p className="text-slate-400 font-medium text-center">엑셀 파일을 업로드하면 스마트 누적 분석 및 <br/> 50MB 이상 대용량 분할 저장이 시작됩니다.</p>
            </div>
          ) : (
            <>
                ...
                {[
                  ...
                  <div key={i} className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-xl shadow-slate-200/40 hover:translate-y-[-4px] transition-all">
                    <div className="flex justify-between items-start mb-6">
                      ...
                      <span className="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em]">통계 요약</span>
                    </div>
                    ...
                    <div className="h-96">
                      <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={dailyTrend}>
                          ...
                          <Area name="조회수" type="monotone" dataKey="조회수" stroke="#3B82F6" strokeWidth={5} fillOpacity={1} fill="url(#colorViewsMain)" />
                        </AreaChart>
                      </ResponsiveContainer>
                    </div>
                  </div>
                ...
                <div className="bg-white rounded-[48px] border border-slate-100 shadow-2xl shadow-slate-200/40 overflow-hidden animate-in zoom-in-95 duration-500">
                  <div className="p-8 bg-slate-50/40 flex flex-col md:flex-row gap-6 border-b border-slate-50">
                    <div className="relative flex-1 group">
                      ...
                      <input type="text" placeholder="상품명 또는 상품 고유번호 검색..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-16 pr-8 py-5 bg-white border border-slate-200 rounded-[28px] focus:outline-none focus:ring-4 focus:ring-indigo-500/5 focus:border-indigo-500 transition-all font-semibold shadow-sm" />
                    </div>
                          ...
                          <tr key={idx} className="hover:bg-slate-50/50 cursor-pointer group" onClick={() => setSelectedProduct(item)}>
                            <td className="px-10 py-8 min-w-[320px]">
                              ...
                              <div className="font-black text-slate-900 group-hover:text-indigo-600 transition-colors flex items-start gap-2 whitespace-normal break-all max-w-[400px] leading-relaxed">
                                {item.lastName} {item.nameCount > 1 && <span className="bg-amber-100 text-amber-700 text-[9px] px-2 py-0.5 rounded-full font-black uppercase shrink-0 mt-1">이름변경</span>}
                              </div>
                              <div className="text-[10px] text-slate-400 mt-2 font-bold tracking-tight">고유번호: {item.상품ID}</div>
                            </td>
                            ...
                            <td className="px-10 py-8 text-center"><div className="w-12 h-12 rounded-2xl bg-white border border-slate-100 flex items-center justify-center text-slate-300 group-hover:bg-indigo-600 group-hover:text-white group-hover:shadow-2xl transition-all mx-auto"><ChevronRight size={18} /></div></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
          ...
          <div className="bg-white w-full max-w-7xl max-h-[94vh] rounded-[64px] shadow-2xl border border-white flex flex-col overflow-hidden animate-in zoom-in-95 duration-700">
              ...
              <div className="flex items-center gap-10">
                ...
                <div className="max-w-[800px]">
                  <div className="flex items-start gap-4">
                    <h3 className="text-3xl font-black text-slate-900 leading-tight tracking-tighter whitespace-normal break-words">{selectedProduct.lastName}</h3>
                  </div>
                  <p className="text-xs text-slate-400 font-bold mt-2 tracking-[0.2em] uppercase opacity-60">상품 고유번호: {selectedProduct.상품ID}</p>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-12 py-12 space-y-16">
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
                {[{ label: '누적 유입수', val: `${selectedProduct.상품상세조회수.toLocaleString()}회`, color: 'blue' }, { label: '누적 매출액', val: `₩${selectedProduct.결제금액.toLocaleString()}`, color: 'indigo' }, { label: '누적 주문량', val: `${selectedProduct.결제상품수량.toLocaleString()}건`, color: 'slate' }, { label: '평균 결제 전환율', val: `${(selectedProduct.상세조회대비결제율 * 100).toFixed(2)}%`, color: 'emerald' }].map((stat, i) => (
                        ...
                        {selectedProduct.performanceByName.map((p, i) => (
                          <tr key={i} className="hover:bg-slate-50/30 transition-colors">
                            ...
                            <td className="px-4 py-6 text-center text-slate-400 text-[11px] font-bold">
                               ...
                               <div className="text-[10px] text-slate-300 mt-0.5">({p.days}일)</div>
                            </td>
              ...
              <div className="space-y-8 pb-10">
                <div className="flex items-center gap-4">
                    ...
                    <h4 className="text-xl font-black text-slate-900 tracking-tight">조회수 타임라인</h4>
                </div>
            ...
            <div className="p-12 bg-white border-t border-slate-50 flex justify-center">
              <button onClick={() => setSelectedProduct(null)} className="px-32 py-6 bg-slate-900 text-white rounded-[32px] font-black text-lg hover:scale-105 active:scale-95 transition-all shadow-2xl shadow-slate-300">닫기</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isProcessing && (
        <div className="fixed inset-0 bg-white/90 backdrop-blur-2xl z-[100] flex flex-col items-center justify-center animate-in fade-in duration-500">
          ...
          <p className="text-slate-400 mt-4 font-black tracking-[0.3em] uppercase text-[11px]">대용량 엔진 가동 중</p>
        </div>
      )}
    </div>
  );
